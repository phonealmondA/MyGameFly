...
// Try just the local path on other platforms
    ...
    while (window.isOpen() && (currentState == GameStateA::MENU || currentState == GameStateA::JOIN_MENU))
    {
        ...
        if (std::optional<sf::Event> event = window.pollEvent())
        {
            ...
            if (event->is<sf::Event::KeyPressed>())
            {
                ...
                if (keyEvent)
                {
                    ...
                    else if (currentState == GameStateA::JOIN_MENU)
                    {
                        ...
                        {
                            // Switch focus between address and port
            ...
            // Handle text input for join menu
            if (event->is<sf::Event::TextEntered>() && currentState == GameStateA::JOIN_MENU)
            {
                ...
                if (textEvent)
                {
                    ...
                    {
                        if (focusAddress)
                            inputAddress += static_cast<char>(textEvent->unicode);
                        else
                        {
                            // Only allow digits for port
                            if (isdigit(static_cast<char>(textEvent->unicode)))
                                inputPort += static_cast<char>(textEvent->unicode);
                        }
                    }
                }
            }
            ...
            if (event->is<sf::Event::MouseButtonPressed>())
            {
                ...
                if (mouseEvent && mouseEvent->button == sf::Mouse::Button::Left)
                {
                    ...
                    // Check if clicking on address or port input boxes
                    if (currentState == GameStateA::JOIN_MENU)
                    {
                        if (addressInput.getGlobalBounds().contains(mousePos))
                            focusAddress = true;
                        else if (portInput.getGlobalBounds().contains(mousePos))
                            focusAddress = false;
                    }
                    ...
                    if (currentState == GameStateA::MENU)
                    {
                        ...
                        else if (hostButton.contains(mousePos))
                            hostButton.handleClick();
                        else if (joinButton.contains(mousePos))
                            joinButton.handleClick();
                    }
                    else if (currentState == GameStateA::JOIN_MENU)
                    {
                        if (connectButton.contains(mousePos))
                            connectButton.handleClick();
        ...
        // Draw menu elements based on current state
        if (currentState == GameStateA::MENU)
        {
